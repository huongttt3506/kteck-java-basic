...
public class D83Collections {
    public static void  main(String[] args) {
        ...
        //2. 앞의 5개의 데이터를 복사 붙어넣기 한 다음
        for (int i = 0; i < 5; i++) {
            newArr[i] = arr[i];
        }
        //3. 새로운 데이터 6을 넣어준다
        newArr[5] = 6;
        //이미 존재 배열에 데이터를 추가하기는 어렵다
        ...
        //get(int index): 몇 번째의 데이터를 사용하겠다. [] 대신 쓴다
        System.out.println(skillList.get(0));
        System.out.println(skillList.get(2));
        //메서드기 때문에 결과 값만 돌려주니까 ...[]로 데이터를 활당하기는 어렵다
//        skillList.get(2) = ""; //컴파이 안되
//        System.out.println(skillList.get(4)); // 지금은 예외 발생
        //특정 위치에 add 하기
        ...
        System.out.println((skillList.contains("git")));
        skillList.remove("Kotlin");
        skillList.remove("Java");
        System.out.println(skillList);

        // Set
        //순서를 보장하는 않으며 중복을 하용하지 않은다, không theo thứ tự, không trùng lặp
        ...
        //이 Iterator가 가르키는 데이터 모음 중, 다음에 볼 뎅;터가 있는지
        // --> 아직 확인할 데이터가 남았는지
        while (listIter.hasNext()) {
            //이 Iterator 가 가르키고 있는 데이터를 가져온다
            System.out.println(listIter.next());
        }
        //Iterator 을 구현한 클레스는
        //for - each 에서 사용이 가능하다
        ...
        //같은 키를 사용하면 덮어씌워진다
        bus.put(9003L, "종료에서 운중동까지");
        bus.put(8100L, "명동에서 정자동까지");
        bus.put(8100L, "명동에서 분당 동까지");
        System.out.println(bus.get(8100L)); //명동에서 분당 동까지
        //없는 데이터를 get 하면, null
        System.out.println(bus.get(7100L)); // null
